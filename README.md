...
- **网易云同步**：通过内置的 [ncm-cli](https://www.npmjs.com/package/@music163/ncm-cli) 拉取收藏的专辑（含曲目列表），支持增量同步与「取消收藏即删除」；同步顺带为存量专辑补全缺失的网易云跳转 ID（仅补空值、不覆盖已有值，完成提示展示补全张数）；同步过程实时显示进度条（拉取阶段显示已获取张数，写入阶段显示 X/Y 比例）
- **专辑墙浏览**：网格视图 + 表格视图自由切换；封面本地缓存（`cover://` 协议）秒开，支持批量补全封面；悬停唱片墙卡片即可一键播放整张专辑；卡片角标（实体图标/我的评分/MB评分/发行日期/已关注）可独立开关显示，按对应字段排序时角标固定显示
...
- **外部站点链接**：详情面板提供 RateYourMusic / Discogs / AllMusic / Last.fm / Wikipedia 跳转，用系统浏览器打开。链接取自 MusicBrainz 的 URL 关系（只存链接、不抓取这些站点的内容）；RYM 入口恒显示且**恒指向 RYM 搜索页**（实测 MB 存的 release 直链点不开，搜索页不依赖 slug 规则、任何专辑都点得进去），其余站点按采集到的直链显示。新专辑随补全顺带写入（不产生额外请求），存量专辑在打开面板时按需采集、每张只查一次
- **搜索**：本地按专辑名 / 艺术家搜索；在线搜索网易云音乐专辑并一键收藏
- **筛选**：艺术家自动补全筛选、多风格 AND 筛选、风格统计面板（点击风格标签即可筛选）
- **关注艺术家**：专辑详情面板中点击艺术家芯片即可关注/取关（表格与唱片墙以纯文本展示，已关注艺术家名以金色文字显示；唱片墙卡片右上角「已关注」★ 角标默认开启、可在角标开关中关闭），工具栏「★ 已关注」一键筛选已关注艺术家的专辑，菜单「工具 → 关注列表」打开独立的关注列表窗口统一管理（专辑数、关注日期、跨窗口实时同步）
- **新专辑动态**：关注列表窗口「动态」Tab 追踪关注艺术家的新发行。自选回溯范围（30 天 / 90 天 / 半年 / 一年）后点击「立即检查」，按发布时间窗增量拉取并按专辑聚合去重，区分「本人名下发行」与「参与作品」（合辑 / OST 弱化展示）；每条显示封面、曲目数与总时长（`1 首 · 3:52` 一眼区分单曲与正式专辑）、发行日期与未读圆点，支持单条 / 全部标记已读，点击 🎵 直达网易云专辑页收藏。已在库的专辑自动跳过，取关时动态一并清理；严格手动触发，不做后台轮询
- **个性化**：用户评分（0.5–5.0）、手动编辑风格标签（候选来自本地风格库，风格仅来自 MusicBrainz 词表，不支持自由创建；候选多时按输入实时筛选并截断为前 50 条）、随机选择「今天听哪张」
- **数据维护**：网易云专辑收藏一致性同步、同步 MusicBrainz 风格库（菜单「数据 → 同步 MusicBrainz 风格库」一次拉全 MB 受控风格词表（2200+）并入本地库，手动分配风格时即可直接搜到，无需改库重启；纯增量写入，不动已有专辑↔风格映射，中断后重跑即续）、扫码登录。封面 / 发行日期 / 艺术家数据不再提供批量回填入口：封面本地缓存未命中时自动拉取落盘，发行日期与结构化艺术家数据随同步、在线添加、单张「重新同步」写入，实测库中已无缺失行
...
> 登录状态自动续期：内置 ncm-cli 0.1.7 起支持 token 过期自动刷新，应用启动时主动续期，正常使用无需频繁重新扫码（长期不使用超出网易云刷新凭证有效期时仍需重新登录）。
...
- 专辑、曲目表及评分、风格等元数据（含专辑的结构化艺术家数据 `artists`（JSON `{name, originalId, id}`，随导出文件保留）与关注艺术家表 `followed_artist`）。专辑还含 `external_links`（外部站点链接 JSON，如 `{"discogs":"..."}`）：NULL = 未回填（打开详情面板时按需采集），`{}` = 已查询过但无链接，两者区分以避免重复查询；该字段随导出文件保留。其中的 `rym` 键照常采集但当前无消费方——详情面板的 RYM 入口恒用搜索页，不读该字段
- 风格库（`genre` 表）：随「同步 MusicBrainz 风格库」扩充至 MB 全量词表，`mb_genre_id` 记录来源 UUID（NULL = 非 MB 同步而来）；导入旧版导出文件时按名称合并、只补空值，不影响既有专辑↔风格映射。风格名的去重统一按「归一化键」（去首尾空白 + 折叠连续空白 + 忽略大小写）比较，写入路径（手动编辑 / MB 补全）与同步路径口径一致，不会再因大小写或空白差异新建重复行；启动迁移会一次性合并历史遗留的重复行（关联改指后删旧行，不丢任何专辑↔风格映射，幂等）
- 新专辑动态条目（`artist_update`）为可重新拉取的派生数据，不纳入导出
- 封面本地缓存
