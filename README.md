...
- **数据维护**：网易云专辑收藏一致性同步、批量补全缺失封面、批量回填缺失发行日期（`ncm-cli album get` 的 publishTime，按北京时间换算，仅填充空值不覆盖已有数据）、批量回填艺术家 ID（老库专辑补齐网易云艺术家 ID，菜单「数据 → 回填艺术家 ID」，回填完成后自动为已关注艺术家补齐 ID）、同步 MusicBrainz 风格库（菜单「数据 → 同步 MusicBrainz 风格库」一次拉全 MB 受控风格词表（2200+）并入本地库，手动分配风格时即可直接搜到，无需改库重启；纯增量写入，不动已有专辑↔风格映射，中断后重跑即续）、扫码登录
...
- 风格库（`genre` 表）：随「同步 MusicBrainz 风格库」扩充至 MB 全量词表，`mb_genre_id` 记录来源 UUID（NULL = 非 MB 同步而来）；导入旧版导出文件时按名称合并、只补空值，不影响既有专辑↔风格映射
