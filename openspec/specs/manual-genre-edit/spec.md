...
用户 SHALL 能够通过自动补全输入框从本地全量风格库中选择风格标签添加到当前专辑。

候选来源 SHALL 为本地 `genre` 表的全部风格（`genre:library`），包含已由「同步 MusicBrainz 风格库」写入但尚未被任何专辑使用的风格。候选 SHALL NOT 包含任何本地不存在的风格——MB 风格库是唯一的风格来源。

#### Scenario: 输入筛选显示匹配风格

- **WHEN** 用户在编辑态输入框中输入文字
- **THEN** 系统 SHALL 显示下拉列表，列出名称包含该文字（不区分大小写）的已有风格，以该文字开头的排在前面

#### Scenario: 候选数量上限

- **WHEN** 输入文字匹配的风格超过 50 个（同步过 MB 风格库后，输入单个字母可能命中数百条）
- **THEN** 下拉列表 SHALL 最多显示 50 条
- **AND** 列表末尾 SHALL 显示「还有 N 个匹配，请继续输入…」，N 为被截断的匹配数
...
- **WHEN** 用户点击编辑按钮进入编辑态
- **THEN** 系统 SHALL 重新拉取一次全量风格库，使补全流程刚自动建档的新风格立即可选

#### Scenario: 不支持创建新风格

- **WHEN** 用户在输入框中输入一个不存在于本地风格库中的文字并按回车
- **THEN** 系统 SHALL NOT 创建新风格，输入框保持不变
- **AND** 用户如需该风格，SHALL 先通过「同步 MusicBrainz 风格库」将其并入本地库
