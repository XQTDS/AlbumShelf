...
面板头部 SHALL 常驻显示；选中专辑时展示专辑名和艺术家信息，并提供 ✕ 按钮；Esc 键 SHALL 同样可以取消选中。

#### Scenario: 头部展示
- **WHEN** 选中一张专辑
- **THEN** 头部 SHALL 显示该专辑的专辑名和艺术家，以及 ✕ 按钮
...
#### Scenario: 完整数据展示
- **WHEN** 用户选中一个已补全的专辑
- **THEN** 面板显示封面图、所有风格标签（旁边有编辑按钮）、MusicBrainz 链接、网易云链接、评分人数、曲目数、同步时间和补全时间
