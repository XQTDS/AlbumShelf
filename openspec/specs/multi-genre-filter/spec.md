...
建议来源 SHALL 仅为「使用中的风格」，即至少关联一张专辑的风格（`album:filters`）。同步 MB 风格库写入但尚未被任何专辑使用的风格 SHALL NOT 出现在筛选建议中——这些风格的筛选结果恒为空。

#### Scenario: 输入匹配
- **WHEN** 用户在输入框中输入 "Ro"
- **THEN** 系统显示使用中的风格里匹配 "Ro" 的建议（如 "Rock", "Rockabilly"）

#### Scenario: 不使用未挂载的风格
- **WHEN** 本地风格库中存在 MB 同步写入但没有任何专辑使用的风格
- **THEN** 该风格不出现在筛选建议列表中
