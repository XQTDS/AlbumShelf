...
- **THEN** 系统 SHALL 使用选中候选的 `id` 和 `originalId` 修复该专辑的 `netease_album_id` 和 `netease_original_id`
...
### Requirement: UNIQUE 约束冲突处理

系统 SHALL 在修复时检查新 `netease_album_id` 是否与其他专辑冲突。

#### Scenario: ID 冲突

- **WHEN** 修复某张专辑的 `netease_album_id`，但新 ID 已被数据库中另一条专辑占用
- **THEN** 系统 SHALL 返回错误信息告知用户冲突，跳过该条修复
