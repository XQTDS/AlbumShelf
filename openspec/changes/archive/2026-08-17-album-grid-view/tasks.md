...
- [x] `App.vue` 样式：新增 `.album-grid`（`repeat(auto-fill, minmax(120px, 1fr))`）、`.album-card`、`.card-overlay`、`.grid-toolbar`、`.cover-placeholder`（复用主题变量与 `--radius`）
- [x] `App.vue` `scrollToAlbumRow`：选择器改 `.album-row, .album-card`，定位改 `getBoundingClientRect` 差值计算
- [x] `App.vue` 排序角标：`cardBadgeText(album)` 按当前排序字段生成角标文本（★ 我的评分 / ⭐ MB评分 / 发行日期，空值不显示）；`.album-card` 新增 `.card-badge`（左下角、z-index 高于遮罩）
...
- 网格视图：封面墙排布随窗口宽度变化（拉宽/拉窄时卡片尺寸与列数同时变化、空间始终铺满无两侧空白）；搜索/筛选结果只有 1-2 张时卡片保持接近 120px 不被拉大；无封面专辑显示 💿；hover 遮罩显示标题+艺术家；点击选中高亮并更新右侧详情面板，再点取消
- 网格排序下拉框：默认/我的评分/MB评分/发行日期各升降序生效；表格列头排序后切到网格，下拉框反映当前排序（反之亦然）；选非默认排序时卡片左下角显示对应字段角标（空值无角标、默认排序无角标、hover 时角标仍可见）
