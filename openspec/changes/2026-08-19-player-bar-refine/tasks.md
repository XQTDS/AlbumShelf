...
- [ ] `src/renderer/src/PlayerBar.vue`：
...
- [ ] 更新 `openspec/specs/playback/spec.md`：「播放控制与状态展示」调整播放条展示内容（歌曲名/艺术家两行、不展示专辑名），新增「信息区固定宽度与超长滚动」「封面点击定位专辑详情」场景
- [ ] 更新 `openspec/specs/album-detail-expand/spec.md`：新增「播放条封面选中专辑」Requirement
- [ ] 同步 `README.md`（内置播放条目：歌曲/艺术家展示、超长滚动、点击封面直达专辑详情）
- [ ] 用户手动 `npm run dev` QA：
